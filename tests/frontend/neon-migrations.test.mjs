...
test('Neon migration workflow is owned by Laravel migrations', () => {
    ...
    assert.equal(
        existsSync('database/migrations/2026_06_16_000000_create_employees_table.php'),
        true,
    );
    assert.equal(
        existsSync('database/migrations/2026_06_16_000001_create_employee_notifications_table.php'),
        true,
    );
});
...
test('employee product tables are defined in Laravel migrations', () => {
    ...
    assert.match(notificationMigration, /Schema::create\('employee_notifications'/);
